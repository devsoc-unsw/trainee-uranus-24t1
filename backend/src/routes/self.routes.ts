...
import { filterAll, filterNullish, filterPublic } from "../services/user-filter";
...
selfRouter.post("/avatar", upload.single('image'), async (req: Request, res: Response) => {
  const file = req.file;
  if (!file) {
  ...
  const result = await collections.users?.updateOne({ _id: req.user._id }, { $set: { avatarUrl: url }});
  if (!result) {
    return res.status(304).send("Could not update user");
  }
...
selfRouter.get("/messages", async (req: Request, res: Response) => {
  const result = await collections.messages?.find({ members: req.user._id }).toArray();
  return res.status(200).json(result);
});

selfRouter.get("/matches", async (req: Request, res: Response) => {
  const result = await collections.users?.find({ $nor: [{ _id: req.user._id }] }).toArray();
