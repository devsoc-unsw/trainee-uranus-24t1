...
import { getSelfData, getStaticData, putSelfAvatar, putSelfData } from "../../backendCommunication";
...
const Profile = () => {
  ...
  const [futureCourseSelection, setFutureCourseSelection] = useState([] as boolean[]);
  const [languageSelection, setLanguageSelection] = useState([] as boolean[]);

  const toggleCourseSelection = (index: number) => setCourseSelection(
    prevState => prevState.map((value, i) => i === index ? !value : value)
  );
  const toggleFutureCourseSelection = (index: number) => setFutureCourseSelection(
    prevState => prevState.map((value, i) => i === index ? !value : value)
  );
  const toggleLanguageSelection = (index: number) => setLanguageSelection(
    prevState => prevState.map((value, i) => i === index ? !value : value)
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFutureCourseSelection(coursesRef.current.map(course => selfData.futureCourses.includes(course)));
    ...
    <div className="relative flex flex-col h-svh w-svw">
      ...
      ">
        <div className={`
          ...
          rounded-2xl
        `}>
          <input type="file" ref={fileInputRef} className="collapse" onChange={async (e: ChangeEvent<HTMLInputElement>) => {
            e.preventDefault();
            ...
            try {
              ...
              await putSelfData(
                token,
                {
                  ...
                  gender,
                  courses: coursesRef.current.filter((_, i) => courseSelection[i]),
                  futureCourses: coursesRef.current.filter((_, i) => futureCourseSelection[i]),
      ...
      <InputModal title="Name" show={nameInputModalShow} onHide={() => setNameInputModalShow(false)}>
      ...
      <InputModal title="Age" show={ageInputModalShow} onHide={() => setAgeInputModalShow(false)}>
        <div className="w-[80%]">
          <LabelledSlider
            min={15}
            max={30}
      ...
      <InputModal title="Gender" show={genderModalInputShow} onHide={() => setGenderModalInputShow(false)}>
      ...
      <InputModal title="Academic Social Ratio" show={asrInputModalShow} onHide={() => setAsrInputModalShow(false)}>
        <div className="w-[80%]">
          <LabelledSlider
            min={0}
            max={1}
      ...
      <InputModal title="WAM" show={wamInputModalShow} onHide={() => setWamInputModalShow(false)}>
        <div className="w-[80%]">
          <LabelledSlider
            min={0}
